...
<style media="screen">
	@media(min-width:1200px) {
		.wcpt-cart-widget {
			display:
				<?php echo $settings['toggle'] == 'enabled' ? 'inline-block' : 'none'; ?>
			;
			<?php
			if (
				...
				?>
				bottom:
					<?php echo $settings['style']['bottom'] . 'px'; ?>
				;
				<?php
			}
			?>
	...
	@media(max-width:1199px) {
		.wcpt-cart-widget {
			display:
				<?php echo $settings['r_toggle'] == 'enabled' ? 'inline-block' : 'none'; ?>
			;
		}
	}
...
?>
<div class="wcpt-cart-widget <?php echo $total_qty ? '' : 'wcpt-hide'; ?>" data-wcpt-href="<?php echo $link_url; ?>">
	<?php echo $style; ?>
	<div class="wcpt-cw-half">
		...
		?>
	</div><a href="<?php echo apply_filters('wcpt_cart_widget_url', $link_url); ?>" class="wcpt-cw-half no-prefetch">
